...
- 番茄钟静默透明态由 `refinement.css` 管理：10 秒无窗口内操作后纸面淡出，导航与计时容器不得另铺底色或投影；仅文字和铅笔笔画带细描边，以适应深浅桌面。正常态与透明态计时 / 进度坐标保持不变。原型备选见 `docs/design/focus-options-20260915/`，用户尚未选择，不当作已实现界面。
...
- `useWindowDrag.ts` 在结束页 `main` 和独立操作提醒的纸面绑定：标题、任务、纸面空白均可拖动；按钮、单选标签、输入、可编辑区域和滚动条排除。鼠标位移串行合并提交，释放/取消/失焦后清理；不抢触控或滚轮滚动。
