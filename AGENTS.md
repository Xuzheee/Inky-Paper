...
- 首页绿色便签的任务文字使用 Xiaolai 20px / 400，行高 1.35，保留原有绿色卡片、居中布局、时长与 start，以及下方清单字体。
- 番茄钟已采用 `docs/design/focus-options-20260915/01-horizontal.png` 横向纸条。`focus-strip.css` 限定 `.focus-strip`：以原 400×210 构图整体缩放到 90%，正常约 360×189、暂停 360×274、随手记 / 菜单 360×359（系统缩放可有 1px 舍入）。`--focus-scale` 与 Rust `FOCUS_SCALE` 保持一致；CSS zoom 已处理百分比宽高，不再反向扩大容器。基础尺寸仍为左侧 Xiaolai 24px 手写任务、右侧 DM Sans 52px 计时（三位分钟 42px），文字、控件与间距一起缩放。长任务至多两行，完整标题通过 title 保留。10 秒无窗口内操作后纸面淡出，导航与计时容器不得另铺底色或投影；透明时使用浅色笔画与细深色描边，计时 / 进度保持原坐标。
...
- 结束页 `EndTask.tsx` 保持 126px 高任务区域，使用用户选择的 C（Xiaolai，小赖）：短任务 42px，字体加载或宽度变化后按实际排版缩至最小 18px；极长文字截断并提供“查看全文”，全文可滚动、Esc 收起。铅笔下划线与截断文字分层，避免露出下一行。完成选项使用 B（Yozai Regular，悠哉常规）18px。两款原始 TTF 和完整 OFL 许可随包保存在 `public/fonts`。`PencilShading.tsx` 用浅铅笔涂抹表示选中，点击后约 480ms 画入；原生单选语义及键盘操作保留，减少动画时直接显示结果。
- `useWindowDrag.ts` 在结束页 `main`、番茄钟横向纸条和独立操作提醒的纸面绑定：标题、任务、纸面空白均可拖动；按钮、单选标签、输入、可编辑区域和滚动条排除。鼠标位移串行合并提交，释放/取消/失焦后清理；不抢触控或滚轮滚动。
- `PaperChrome.tsx` 的固定透明层提供左侧 10px 和底部 6px 留白拖动区，滚到任意位置仍可拖窗，位于任务行之外。主页面与首页 `home-body` 的原生滚动条隐藏，由 `PaperScrollbar.tsx` 提供 3px 浮动滑块、10px 透明抓取区；滚动、悬停或键盘聚焦时显示，闲置淡出，不预留宽度。支持拖动、滚轮、轨道翻页及方向 / Page / Home / End 键；无溢出时不显示。该层挂在 body，不受番茄钟 zoom 影响；迷你宠物卸载，独立提醒与工作台不挂载。输入框与全文阅读保留原生细滚动条。
- `PaperFooter.tsx` 共用原鱼和八句英文随机标语，首页、结束页、休息中和休息结束共用同一次启动选中的值。结束页为 320×528，展开记录也不改变窗口尺寸；休息页为 320×430。结束页和休息页页脚最小 53px，底部约 6–8px。选项下说明保存后的去向：未完成回列表，已完成到完成页再选回列表或休息；完成页提醒整个任务单独完成。休息返回列表保留计时，结束休息不自动开始下一段。
