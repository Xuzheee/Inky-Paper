...
- `PaperFooter.tsx` 共用原鱼和八句英文随机标语，首页、结束页、休息中和休息结束共用同一次启动选中的值。结束页为 320×528，展开记录也不改变窗口尺寸；休息页为 320×430。结束页和休息页页脚最小 53px，底部约 6–8px，纸面滚动条为 4px。选项下说明保存后的去向：未完成回列表，已完成到完成页再选回列表或休息；完成页提醒整个任务单独完成。休息返回列表保留计时，结束休息不自动开始下一段。
