...
当前应用版本 **0.5.10**，Inky Coach 插件版本 **0.1.3**。当前进度、验证范围和待检查项见 [项目进度](docs/PROJECT_STATUS.md)。
...
番茄钟运行时，10 秒未在窗口内操作会淡出纸面与辅助控件，留下计时、铅笔进度和返回 / 结束入口；透明态不再残留两块底色。移动鼠标或按键恢复纸面，暂停或编辑随手记期间保持显示。
