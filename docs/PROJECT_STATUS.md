...
更新日期：2026-09-16。当前纸面分支应用 **0.6.1**；Hermes Inky Coach 插件 **0.1.3**。本文记录已实现能力、已有验证和后续检查，不把历史验收当作本轮重跑结果。
...
类型检查、114 项前端测试与 release 构建通过。独立 Tauri / WebView2 检查覆盖顶部 / 中部 / 底部拖动、零滚动条占位、滑块拖动 / 滚轮 / 键盘、大小任务划线、记录输入及保存、页面切换、透明态和迷你宠物；见 [拖动与滚动验收](verification/paper-chrome-20260916/README.md)。当前分支的可执行文件已同步，桌面正式版仍待两分支合并后统一更新。
...
类型检查、104 项前端测试、67 项 Rust 测试通过，1 项历史负载基准忽略。隔离 Tauri / WebView2 验证覆盖短任务、长中文和 300 字中英文换行文本、真实字体、全文阅读、拖动、选项、草稿恢复、两种保存结果、休息与 Markdown 回流。证据见 [结束页验收](verification/end-layout-20260915/README.md)。此改动只在本 worktree 交付，待与工作台分支合并验收后统一更新桌面正式版。
