...
const out = path.resolve('output/focus-transparency-acceptance');
const shots = path.resolve('output/playwright/focus-transparency');
...
const snapshot = () => page.evaluate(() => {
  ...
  return {
    ...
    timer: rect('.timer'), progress: rect('.pencil-progress'),
...
try {
  ...
  await api('start_session', { taskId: task.id, expectedRevision: task.revision, kind: 'focus', plannedSeconds: 1500 });
  await page.reload();
  ...
  assert.equal(normal.width, 320); assert.equal(normal.height, 196);
  ...
  assert.equal(transparent.timerStroke, '1.5px');
  ...
  pass('Native 320x196 focus window fades all paper surfaces; timer/progress stay in place and keep advancing, navigation remains available');
  ...
  await writeFile(path.join(out, 'verification.json'), JSON.stringify({ result: 'PASS', checks, normal, transparent, errors, isolatedData: true }, null, 2));
} catch (error) {
