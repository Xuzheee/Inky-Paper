use super::*;
fn request(mut value: Value) -> Value { value["requestId"] = json!(id()); value }
...
fn save(c: &mut Connection, date: &str) -> Value {
    call(c, "workbench_save_step", json!({"taskId":id(),"stepId":id(),"title":"合成计划","text":"画草图","category":"work","priority":"high","due":"客户确认后","dueDate":"2030-03-08","expectedResult":"三列布局可检查","plannedSeconds":900,"date":date}))
}
...
fn optional_metadata_roundtrips_and_invalid_date_rolls_back() {
    ...
    for date in ["2030-02-30", "2030-3-04", "明天"] {
        let invalid = request(json!({"taskId":initial["task"]["id"],"expectedRevision":2,"patch":{"dueDate":date}}));
        assert!(execute(&mut c,"update_task",invalid,"user").unwrap_err().contains("INVALID_INPUT"));
        assert_eq!(serde_json::to_value(load(&c).unwrap()).unwrap(),before);
    }
    let clear = call(&mut c,"update_task",json!({"taskId":initial["task"]["id"],"expectedRevision":2,"patch":{"dueDate":null}}));
...
fn continue_reuses_existing_target_and_retry_preserves_history() {
    ...
    assert_eq!(after["planning"]["dayItems"][0]["continuedTo"],first["item"]["id"]);
    assert_eq!(after["planning"]["planChanges"].as_array().unwrap().len(),3);
    ...
    let older = call(&mut c,"workbench_save_step",json!({"taskId":original["task"]["id"],"stepId":original["step"]["id"],"expectedTaskRevision":1,"expectedStepRevision":1,"title":"合成计划","text":"画草图","category":"work","plannedSeconds":900,"date":"2030-03-01"}));
    let mut again = item_request(&older); again["date"]=json!("2030-03-04");
...
fn cancellation_checks_complete_unexecuted_set_and_keeps_executed_source() {
    ...
    let mut go = item_request(&old); go["date"]=json!("2030-03-04");
    let today = call(&mut c,"continue_plan_items",go);
    let session = call(&mut c,"start_session",json!({"taskId":old["task"]["id"],"expectedRevision":1,"stepId":old["step"]["id"],"expectedStepRevision":1,"dayItemId":old["item"]["id"],"kind":"focus","plannedSeconds":900}));
    let mut cancellation = item_request(&old); cancellation["scope"]=json!("unexecuted");
    cancellation["items"]=json!([{"id":old["item"]["id"],"revision":2},{"id":today["item"]["id"],"revision":1}]);
    let before = serde_json::to_value(load(&c).unwrap()).unwrap();
    assert!(execute(&mut c,"cancel_plan_items",request(cancellation.clone()),"user").unwrap_err().contains("CONFLICT"));
    assert_eq!(serde_json::to_value(load(&c).unwrap()).unwrap(),before);
...
fn prepared_source_persists_without_clock_and_active_clock_rejects_replacement() {
    ...
    drop(c);
    let mut c=open(&path).unwrap();
    assert_eq!(json!(load(&c).unwrap().planning.prepared.unwrap())["dayItemId"],out["item"]["id"]);
    call(&mut c,"start_session",json!({"taskId":out["task"]["id"],"expectedRevision":1,"stepId":out["step"]["id"],"expectedStepRevision":1,"dayItemId":out["item"]["id"],"kind":"focus","plannedSeconds":900}));
    let before=serde_json::to_value(load(&c).unwrap()).unwrap();
    assert!(execute(&mut c,"prepare_step",request(input),"user").unwrap_err().contains("ACTIVE_SESSION"));
    assert_eq!(serde_json::to_value(load(&c).unwrap()).unwrap(),before);
    let record=execute(&mut c,"get_daily_record",json!({"date":"2030-03-06","utcOffsetMinutes":480}),"user").unwrap().0;
    assert_eq!(record["planChanges"].as_array().unwrap().len(),1);
    assert!(std::fs::read_to_string(dir.path().join("工作记录/任务.md")).unwrap().contains("截止日期：2030-03-08"));
}
